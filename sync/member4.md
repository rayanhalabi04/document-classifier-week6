...
- **T042** — GitHub Actions CI pipeline — format, lint, typecheck, unit tests (`.github/workflows/ci.yml`) (later rewritten by Member 1)
- **T044** — Golden-set + compose smoke CI — spins up the docker-compose stack, waits for service health, then runs the deterministic golden-set replay against the classifier (`.github/workflows/ci.yml`, `backend/tests/golden/test_golden_replay.py`)
