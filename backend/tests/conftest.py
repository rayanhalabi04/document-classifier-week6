...
# ── Block real SFTP connections ────────────────────────────────────
#
# Unit and contract tests must never make real network calls.  This
# autouse fixture patches paramiko everywhere and raises on any attempt.
# Integration tests (tests/integration/) bypass the patch — they are the
# only tests allowed to connect to real Docker services.


@pytest.fixture(autouse=True)
def _block_real_sftp_connections(request):
    if "tests/integration" in str(request.fspath):
        yield
        return

    with patch(
        "paramiko.Transport",
        side_effect=RuntimeError("Real SFTP blocked — does this test need Docker?"),
    ), patch(
